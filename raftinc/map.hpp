...
public:
    ...
    void exe()
    {
        ...
        if( dot_graph_env != nullptr )
        {
            ...
            raft::make_dot::run( of, all_kernels, source_kernels );
            of.close();
    ...
    kernel_pair_t operator+=( kpair &p )
    {
        ...
        auto next_link_type( []( const kpair * const k ) -> sj_t
        {
            ...
            {
                throw InvalidTopologyOperationException(
                        "Invalid ordering of topology operators \"<=\","
...
protected:
                            ...
                            kernelkeeper &all )
    {
        ...
        /** don't have to do this but it makes it far more apparent
         * where it comes from **/
        void * const kernel_ptr( reinterpret_cast< void* >( &all_k ) );
        using kernel_ptr_t = std::remove_reference< decltype( all_k ) >::type;
        /** need to grab impl of Lengauer and Tarjan dominators, use for **/
        /** SESE in the interim, restrict to kernels that are simple to **/
        /** duplicate. **/
        ...
        GraphTools::BFS( source_k,
                         ...
                         {
                             auto * const all_k(
                                     reinterpret_cast< kernel_ptr_t* >( data )
                                     );
                             if( a.out_of_order && b.out_of_order )
                             {
                                     ...
                                     a.my_kernel->dup_candidate )
                                 {
                                     ...
                                     auto &port_info_front(
                                             kernel_a->input.getPortInfo() );
                                     auto *front(
                                             port_info_front.other_kernel );
                                     auto &front_port_info(
                                             front->output.getPortInfo() );
                                     /**
                                     ...
                                     /**
                                      * now we need the port info from the
                                      * input port on back
                                      **/
                                      ...
                                      * kernel_a -> join -> back
                                      */
                                     auto *join( static_cast< raft::kernel* >(
                                                 a.join_func() ) );
                                     all_k->insert( join );
                                 ...
                                 /** parallalizable source, single output no
                                  * inputs**/
                                 ...
                                 /** parallelizable sink, single input, no
                                  * outputs **/
                                 else if( b.my_kernel->input.count() == 1 &&
                                          b.my_kernel->output.count() == 0 )
                                 {
                                     auto *split( static_cast< raft::kernel* >(
                                            b.split_func() ) );
                                     all_k->insert( split );
                                     MapBase::insert( a.my_kernel, a,
                                                      b.my_kernel, b,
...
private:
                      ...
                      kpair * const next )
    {
        ...
        {
            /**
             * need to add exception for too many output ports for
             * source, or too few input ports for destination
             */
